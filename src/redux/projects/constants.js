export const GET_PROJECTS_SUCCESS = 'GET_PROJECTS_SUCCESS';
export const GET_PROJECTS_PENDING = 'GET_PROJECTS_PENDING';
export const GET_PROJECTS_ERROR = 'GET_PROJECTS_ERROR';
export const DELETE_PROJECT_ERROR = 'DELETE_PROJECT_ERROR';
export const DELETE_PROJECT_SUCCESS = 'DELETE_PROJECT_SUCCESS';
export const DELETE_PROJECT_PENDING = 'DELETE_PROJECT_PENDING';
export const SET_INFO_FOR_FEEDBACK = 'SET_INFO_FOR_FEEDBACK';
export const GET_ID_FROM_ROW = 'GET_ID_FROM_ROW';
export const SHOW_DELETE_MESSAGE = 'SHOW_DELETE_MESSAGE';
export const SHOW_FEEDBACK_MESSAGE = 'SHOW_FEEDBACK_MESSAGE';
export const SHOW_PRELOADER = 'SHOW_PRELOADER';
export const POST_PROJECT_ERROR = 'POST_PROJECT_ERROR';
export const POST_PROJECT_SUCCESS = 'POST_PROJECT_SUCCESS';
export const POST_PROJECT_PENDING = 'POST_PROJECT_PENDING';
export const EDIT_PROJECT_SUCCESS = 'EDIT_PROJECT_SUCCESS';
export const EDIT_PROJECT_ERROR = 'EDIT_PROJECT_ERROR';
export const EDIT_PROJECT_PENDING = 'EDIT_PROJECT_PENDING';
export const GET_SELECTED_PROJECT = 'GET_SELECTED_PROJECT';
export const CLEAN_SELECTED_PROJECT = 'CLEAN_SELECTED_PROJECT';
export const SET_INFO_TO_SHOW = 'SET_INFO_TO_SHOW';
export const SHOW_INFO = 'SHOW_INFO';
export const EDIT_PROJECT_STATUS_PENDING = 'EDIT_PROJECT_STATUS_PENDING';
export const EDIT_PROJECT_STATUS_ERROR = 'EDIT_PROJECT_STATUS_ERROR';
export const EDIT_PROJECT_STATUS_SUCCESS = 'EDIT_PROJECT_STATUS_SUCCESS';
